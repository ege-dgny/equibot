...
DATA_PATH="${DATA_PATH:-/home/hikmet/equibot/data/can/pcs}"
PREFIX="${PREFIX:-can_equibot_100d}"
LOG_DIR="${LOG_DIR:-/home/hikmet/equibot/logs/train}"
...
env NUMBA_CACHE_DIR="${NUMBA_CACHE_DIR:-/tmp/numba_cache}" \
    ...
    conda run --no-capture-output -n lfd python -m equibot.policies.train \
        --config-name can_equibot \
        use_wandb="${USE_WANDB}" \
        prefix="${PREFIX}" \
        log_dir="${LOG_DIR}" \
